...
Status: implementation in progress and authorized on 2026-09-21. Milestones A/B and the first bounded C/D/E slices have been deployed locally; this document remains the decision framework, not a claim of profitable alpha.
...
- Added temporal archetype and common ML evaluators with immutable manifests/trial ledgers, prior-only fitting/calibration, causal baselines, block-bootstrap intervals and fail-closed promotion gates.
- Executed the common BTC 4h next-bar evaluator on 14,319 source rows: 114 outer folds and 13,680 identical OOS timestamps. `hist_gradient_boosting` beat the strongest rolling-class-probability baseline on Brier score (0.465 vs 0.573; paired lift 0.108, familywise CI 0.101 to 0.116) and log loss (0.790 vs 0.968). This passes the predeclared predictive evidence gate only; it is not an economic, forward-paper or live-trading promotion.
...
- Unified backtest/replay execution semantics. Historical replay is explicitly not prospective paper evidence; forward paper remains unavailable until a timestamped live fill recorder exists.
